...
public class RavagerItem extends GunItem {
    public RavagerItem(Properties properties) {
        super(properties.durability(2107));
    }
    
    @Override
    public boolean isValidRepairItem(ItemStack toRepair, ItemStack repair) {
        return repair.is(DLItems.RAVAGING_INGOT.get());
    }
    ...
    public InteractionResultHolder<ItemStack> use(@NotNull Level level, Player player, @NotNull InteractionHand hand) {
        ...
        boolean flag = player.isCreative() || ItemUtils.getEnchantmentLevel(registryAccess, gunStack,
                registryAccess.registryOrThrow(Registries.ENCHANTMENT)
                        .getResourceKey(Enchantments.INFINITY_ARROWS)
                        .orElseThrow()
        ) > 0;
        
        ItemStack ammo = findAmmo(player, new ItemStack(DLItems.UMBRIUM_PELLET.get()));
